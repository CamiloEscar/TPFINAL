...
procedure menu_ABMC;
procedure cantidad_infectado_nacional (var arch:fichero_per;var cant:integer);
...
procedure cantidad_edad_nacional (var arch:fichero_per; Var cant1,cant2,cant3:integer);
procedure cantidad_infectado_Provincial (var arch:fichero_per;cod_est:byte; var cant:integer);
procedure cantidad_estado_provincial (var arch:fichero_E; cod_est:byte; Var cant1,cant2,cant3:integer);
procedure cantidad_forma_provincial (var arch:fichero_E; cod_est:byte; Var cant1,cant2,cant3:integer);
procedure cantidad_edad_provincial (var arch:fichero_per; cod_est:byte; Var cant1,cant2,cant3:integer);
procedure listar_I(var arch:fichero_per;verdadero:boolean);
procedure listar_i_estancias(var arch:fichero_per; auxCodProv:byte;aux_denominacion:string);
...
begin
	...
	writeln ('--------------------------------------------------------------------------------');//01
	writeln ('- Datos del Visitante:                                                         -');//02
	writeln ('--------------------------------------------------------------------------------');//03
...
begin
	assign(arch, ruta_Per); // assign: es la orden que se encarga de asignar un nombre físico al fichero que acabamos de declarar.
...
begin
	...
	while not (eof(arch))do
		begin
			read (arch,Reg);
			if reg.condicion=true then
				if (reg.codigo_estancia=cod_est) then
					inc(Cant)
		end;
end;
...
procedure orden_burbuja_I (var arch:fichero_per);
var 
	reg1,reg2:personas;
	lim,i,j:integer;
begin
...
begin
	...
	while not (eof(arch))do
		begin
			read (arch,Reg);
			if reg.condicion=true then
				if reg.codigo_estancia=auxCodProv then
					mostrar_registro_per_Provincia(reg);
		end;
end;

// Procedimiento para mostrar los pacientes
procedure mostrar_registro_per(registro:personas);
begin
	writeln (registro.dni,',',registro.apellidoynombre,', Cod Prov: ',registro.codigo_estancia);
end;
...
procedure modificar_alta_baja_I(var arch:fichero_per;pos:integer; verdad:boolean);
var
	reg:personas;
begin
	seek(arch,pos); // Se posiciona en el registro con posición pos
...
procedure buscar_I(var arch:fichero_per; documento:string; var encon:boolean);
var
	reg:personas; 
	i: integer;
begin
			...
			with reg do
				begin 
					...
					repeat
						...
						prov:=codigo_estancia;
					until (codigo_estancia >0) and (codigo_estancia <= cantidad_estancias);
					buscar_registro_est (archivo_est,registro_est,codigo_estancia,encontrado,ubicacion);
					mostrar_en_carga (archivo_est,registro_est,codigo_estancia,ubicacion,aux_denominacion);
					gotoxy (18,14);
...
begin
	...
	seek(archivo_per,ubic); // Se posiciona en el registro con posición pos
...
begin
	repeat
		...
		orden_burbuja_I (archivo_per); // ordenda los registros por dni
		orden_burbuja_E (archivo_E); // ordenda los registros por dni
		cerrar_archivo_per (archivo_per); // cierra el archivo
					...
					if ubicacion <> -1 then
						begin
							...
							if Ingresar='S' then
								begin
									aux_condicion:=false;
									modificar_alta_baja_I (archivo_per,ubicacion,aux_condicion); // Cambia la condición true a false
					...
					if ubicacion <> -1 then
						begin
							...
							if Ingresar='S' then
								begin
									aux_condicion:=true;
									modificar_alta_baja_I (archivo_per,ubicacion,aux_condicion); // Cambia la condición false a true
