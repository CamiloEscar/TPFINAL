...
procedure mostrar_registro_est(var arch:fichero_est; reg:estancias);
begin
...
procedure mostrar_registro_cargado(var arch:fichero_per; reg:personas; documento:string; ubi:integer);
begin
...
begin
	...
	if pos <> -1 then
		begin
			mostrar_registro_cargado(archivo_per,registro_per,documento,pos);
		end;
end;
...
begin
	...
	with reg do
		begin
			...
			readln(denominacion);
			gotoxy(1,40);
			writeln ('Ingrese el codigo provincia de la Estancia      ');
			gotoxy(13,3+codigo_provincia);
...
// Procedimiento para cargar las estancias
procedure cargar_registro_duenio(var reg:personas; var documento:string; var prov:byte;var ubic:integer);
var
...
procedure modificar_registro(var reg:personas; var documento:string; var prov:byte;var ubic:integer);
var
	cantidad_estancias:byte;
	validar_fecha:boolean;
	ubicacion:integer;
		...
		Case opcion of
			'1': begin // Alta de dueño
					repeat
						abrir_archivo_per (archivo_per);
                        abrir_archivo_est (archivo_est); 
						cargar_registro_duenio (registro_per,aux_dni,aux_provincia,ubicacion); // Carga los datos de un registro
						...
						if ubicacion<>-1 then // si ya existe muestra los datos
							begin
                                ...
                                mostrar_registro_est(archivo_est,registro_est);
								gotoxy(1,24) ;
			...
			'3': begin // Modificación de visitante
					...
					if ubicacion <> -1 then
						begin
							...
							if Ingresar='S' then
								begin
									modificar_registro (registro_per,aux_dni,aux_provincia,ubicacion); // Muestra los datos a modificar
								end;
						end;
					cerrar_archivo_per (archivo_per);
                    //cerrar_archivo_est (archivo_est); 
